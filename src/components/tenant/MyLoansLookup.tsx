    ...
    <main className="min-h-screen px-6 py-14">
      <div className="mx-auto max-w-4xl space-y-8">
        <div className="flex justify-start">
          <Link href={librarySlug ? `/l/${librarySlug}` : '/'} className="rounded-full bg-white border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50 shadow-xs transition">
        ...
        <form onSubmit={handleSubmit} className="rounded-4xl border border-slate-200 bg-white p-6 shadow-[0_30px_80px_-45px_rgba(15,23,42,0.25)]">
        ...
        {loans.length > 0 && (
          <div className="flex bg-slate-100 p-1 rounded-2xl w-fit mx-auto md:mx-0">
            <button onClick={() => setActiveTab('pending')} className={cn('px-4 py-2 text-sm font-semibold rounded-xl transition', activeTab === 'pending' ? 'bg-white shadow-xs text-slate-900' : 'text-slate-500 hover:text-slate-700')}>Pendientes</button>
            <button onClick={() => setActiveTab('active')} className={cn('px-4 py-2 text-sm font-semibold rounded-xl transition', activeTab === 'active' ? 'bg-white shadow-xs text-slate-900' : 'text-slate-500 hover:text-slate-700')}>Activos</button>
            <button onClick={() => setActiveTab('overdue')} className={cn('px-4 py-2 text-sm font-semibold rounded-xl transition', activeTab === 'overdue' ? 'bg-white shadow-xs text-slate-900' : 'text-slate-500 hover:text-slate-700')}>Vencidos</button>
            <button onClick={() => setActiveTab('finished')} className={cn('px-4 py-2 text-sm font-semibold rounded-xl transition', activeTab === 'finished' ? 'bg-white shadow-xs text-slate-900' : 'text-slate-500 hover:text-slate-700')}>Finalizados</button>
          </div>
        )}

        <div className="space-y-4">
          {filteredLoans.map((loan) => (
            <article key={loan.id} className="rounded-4xl border border-slate-200 bg-white p-6 hover:border-slate-300 transition shadow-xs">
